"""
app/core/database.py — Async Database Engine & Multi-Tenant Session Management
...
async def get_public_db() -> AsyncGenerator[AsyncSession, None]:
    """
    Yields an async session pinned to the `public` schema.
...
async def get_tenant_db(request: Request) -> AsyncGenerator[AsyncSession, None]:
    """
    Yields an async session pointed at the current tenant's schema.

    Reads `request.state.tenant_id` (set by TenantMiddleware) and issues
    SET search_path before yielding. All ORM calls are automatically isolated
...
async def provision_tenant_schema(tenant_id: str) -> str:
    ...
    from app.plugins.inventory.models import Item, Invoice, InvoiceLine
    ...
    async with engine.begin() as conn:
        ...
        # Point all subsequent DDL at the new schema
...
_BYPASS_PATHS = {"/docs", "/redoc", "/openapi.json", "/health", "/favicon.ico"}
_BYPASS_PREFIXES = (
    "/api/v1/system/auth/",
